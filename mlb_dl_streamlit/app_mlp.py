...
from rolling_features import build_game_features_from_history
...
def load_model():
    state = torch.load("models/model_roll.pt", map_location="cpu")
    scaler = load("models/scaler_roll.joblib")
    with open("models/feature_cols_roll.json","r", encoding="utf-8") as f:
        cols = json.load(f)
